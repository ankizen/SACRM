...
## Phase 5 — Frontend Feature Screens ✅ (done)
...
- **5b ✅ — Lead detail sub-resources.** Activities (append-only log form + list), Notes (add/inline-edit/delete), Followups (schedule + status Select that PUTs Completed/Missed/Cancelled), Attachments (upload with category, authenticated blob download via a temporary object URL since a plain `<a href>` can't carry the JWT, delete) — all four wired to Phase 3b's endpoints and reusing the `PagedResult<T>` type factored out of `features/leads/types.ts` into `lib/types.ts` in this checkpoint. None of these actions are Admin+-gated on the frontend because Phase 3b's backend never restricted them beyond lead-visibility scoping (unlike Leads' own Assign/Delete/Merge) — Executives can log activities/notes/followups/attachments on their own leads same as anyone else.

  Verified end-to-end with the same Playwright-driver approach: logging an activity, adding then inline-editing a note, scheduling a followup and marking it Completed, and uploading then downloading an attachment (byte-for-byte content check on the downloaded file) all worked, and the Timeline tab confirmed every one of them auto-wrote the matching audit entry (ActivityLogged, NoteAdded, FollowupScheduled, FollowupCompleted, AttachmentAdded) — Phase 2's interceptor doing exactly what it was built for.
- **5c ✅ — Everything else.** Settings (`/settings`, tabbed: Company Profile — Master-Admin-edit-only, Lead Stages, Lead Sources, Countries, Cities, all reusing the read hooks from 5a with a new `includeInactive` param that defaults `false` so the Lead form/filter dropdowns are unaffected). Users (`/users`, create Admin/Executive with the same role restriction the backend enforces, deactivate toggle). Reports (`/reports`, groupBy selector over `leads-summary` rendered as proportional bars, plus a `followups-summary` tile). Dashboard gained an Executive Performance table for Admin+. Import/Export UI: a multi-step dialog (upload → preview with auto-guessed column mapping → confirm → result summary) and an Export button next to the Lead list's existing filters, both on `LeadListPage`.

  Two real bugs found and fixed during verification:
  1. **Backend**: creating or renaming a LeadSource/LeadStage/Country/City to a name that already exists hit the database's unique-index constraint and bubbled up as a raw 500 instead of a clean 409 — none of the four Phase 3a lookup controllers pre-checked for an existing name before insert/update, unlike `UsersController.Create` which already did this correctly. Added the same `AnyAsync` pre-check (scoped to `(CountryId, Name)` for City, matching its composite unique index) to all four controllers' Create and Update actions. Also added a `getErrorMessage()` helper (`lib/errors.ts`) so the frontend surfaces the backend's actual `ConflictException` message instead of a generic "could not save."
  2. **Frontend**: the Import dialog's "map at least Name and Phone" validation checked `mapping.Name`/`mapping.Phone` — but `mapping` is keyed by *CSV header* (e.g. `"Full Name"`) with the target field name as the *value*, so that check was looking for a column literally named "Name", not for whether anything had been mapped to the Name field. The Import button stayed disabled even with a fully valid mapping. Fixed by checking `Object.values(mapping)` instead.

  Verified end-to-end: settings CRUD (including the now-clean duplicate-name rejection), user creation, reports with groupBy switching, dashboard executive performance, and a full import (CSV upload → auto-guessed + manually-completed column mapping → created leads appearing in the list) and export (downloaded file, correct size) round-trip.
